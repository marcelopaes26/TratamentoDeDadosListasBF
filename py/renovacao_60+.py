...
df_tratados['FONES'] = df_tratados.apply(
    lambda row: selecionar_telefone(row['DDD'], row['Fone 01'], row['Fone 02'], row['Fone 03']),
    axis=1
)
...
df_tratados['NOME'] = df_tratados['Cliente'].apply(limpar_nome)
...
# Colunas ordenadas
df_tratados = df_tratados[['REGIONAL', 'NOME AGENTE', 'TIPO CONTRATO', 'CONTRATO', 'NOME', 'TIPO CLIENTE', 'PRODUTO', 'VLR EMPRESTADO', 'CARTEIRA ATIVA', 'FONES']]
