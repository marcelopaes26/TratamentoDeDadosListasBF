...
origem = 'C:/Users/diego.pissetti/Documents/Marcelo/Python/TratamentoDeDadosListasBF/origem/Cobrança - Clientes Mora Vira Risco 07-2025.xlsx'
destino = 'C:/Users/diego.pissetti/Documents/Marcelo/Python/TratamentoDeDadosListasBF/destino/Cobrança - Clientes Mora Vira Risco 07-2025.xlsx'

# Força leitura de colunas como texto para evitar notação científica
colunas_texto = ['CPF_CNPJ', 'DDD', 'Fone 1', 'Fone 2', 'Fone 3', 'Fone 4', 'Fone 5', 'TELEFONE', 'Contrato', 'CONTRATO']
...
def selecionar_telefone(ddd, f1, f2, f3, f4, f5):
    ddd = ''.join(filter(str.isdigit, str(ddd))) if pd.notna(ddd) else ''
    fones = [f1, f2, f3, f4, f5]
...
# Remove o dígito após o número do contato
def limpar_contrato(contrato):
    if (pd.isna(contrato)):
        return ''
    return f'{contrato[:-2]}'

# Retirar os contratos de Convênio, Óbito (Carteira Sede) e Avalistas
df_tratados = df_tratados[~df_tratados['Produto'].isin(['BF CONVÊNIO']) &
                          ~df_tratados['Cód. Carteira'].isin(['BF-SEDE']) &
                          ~df_tratados['Tipo Cadastro'].isin(['Avalista']) &
                          ~df_tratados['Tipo Cadastro'].isin(['O'])]

# Chama a função limpar_contrato() no Data Frame
df_tratados['CONTRATO'] = df_tratados['Contrato'].apply(limpar_contrato)
...
df_tratados['FONES'] = df_tratados.apply(
    lambda row: selecionar_telefone(row['DDD'], row['Fone 1'], row['Fone 2'], row['Fone 3'], row['Fone 4'], row['Fone 5']),
    axis=1
)
...
# Colunas ordenadas
df_tratados = df_tratados[['REGIONAL', 'UNIDADE', 'CÓD. CARTEIRA', 'CONTRATO', 'VLR', 'NOME', 'FONES']]
